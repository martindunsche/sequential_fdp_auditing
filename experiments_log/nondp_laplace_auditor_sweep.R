...
rlaplace1 <- function(scale) {
  # Match rmutil::rlaplace(1, m = 0, s = scale).
  # Use rmutil when installed; otherwise use the inverse-CDF sampler.
  if (requireNamespace("rmutil", quietly = TRUE)) {
    return(rmutil::rlaplace(1L, m = 0, s = scale))
  }
  u <- stats::runif(1L, min = -0.5, max = 0.5)
  -scale * sign(u) * log1p(-2 * abs(u))
}

make_nonDP_laplace <- function(eps) {
  # NonDPLaplace1 from the comparison benchmark: true sample size n is used
  # in both the mean and the noise scale.
  function(x) {
    n <- length(x)
    mean_nonpriv <- sum(x) / n
    rho <- rlaplace1(scale = 2 / (n * eps))
    mean_nonpriv + rho
  }
}

make_puredp_tradeoff <- function(eps) {
  force(eps)
  function(alpha) {
    pmax(
      0,
...
run_missing_trials_for_eps <- function(
  ...
  boundary_tweaks, mc_cores, show_progress, chunk_size, seed
) {
