...
def _get_sigma_gaussian(eps: float, delta: float, sensitivity: float = 1.0) -> float:
    """
    Computes the exact sigma for the Gaussian mechanism using the 
    Analytic Gaussian Mechanism formula (Balle & Wang, 2018).
    """
    if eps <= 0 or delta <= 0:
        return 0.0
    ...
    def objective(sigma):
        if sigma <= 0:
            return 1.0 # arbitrary large delta
        
        
        val = sensitivity / (2 * sigma)
        term_1 = norm.cdf(val - eps * sigma / sensitivity)
        term_2 = np.exp(eps) * norm.cdf(-val - eps * sigma / sensitivity)
        
        computed_delta = term_1 - term_2
    ...
    # Expand upper bound until we find a sigma large enough to satisfy delta
    while objective(sigma_hi) > 0: 
        sigma_hi *= 2
        if sigma_hi > 1e8: # Safety break
            return 1e8

    # Use Brent's method to find the root
    try:
        sigma = optimize.brentq(objective, sigma_lo, sigma_hi, xtol=1e-10)
...
def _gaussian_dp_blow_up_inverse(
  ...
  # Try to use dp_accounting API first, fall back to our implementation
  try:
    # Check if dp_accounting has the function (for newer versions)
    if hasattr(dp_accounting, 'get_sigma_gaussian'):
      sigma = dp_accounting.get_sigma_gaussian(eps, delta)
    else:
      # Use our implementation for dp_accounting 0.5.1+
      sigma = _get_sigma_gaussian(eps, delta)
